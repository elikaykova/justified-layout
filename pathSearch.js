...
function resize(start, end, height) {
    ...
    for (var i = start; i < end - 1; i++) {
        let currWidth = Math.floor(
            scalingDownToFit(height, data[i].height, data[i].width)
        );
...
function goTroughtGroups(start, end) {
    ...
    for (var i = start; i < end; i++) {
        currWidth += Math.floor(
            scalingDownToFit(idealRowHeight, data[i].height, data[i].width)
        );
        //console.log("photo(" + i + ") : ");
...
function findWeight(start, end) {
    ...
    let currHeight = findBestHeight(currWidth);
    let badness = Math.abs(idealRowHeight - currHeight);
    console.log("weight from " + start + " to " + end + ": " + badness);
...
function findChildren(currIndex) {
    ...
    while (widthSum < viewport && currIndex < photoNumber) {
        widthSum += Math.floor(
            scalingDownToFit(
                idealRowHeight,
                data[currIndex].height,
                data[currIndex].width
            )
        );
    ...
    while (notFound || notDouble) {
        ...
        for (var i = 0; i < 2; i++) {
                ...
                if (res)
                    console.log(
                        "ResBadness: " +
                            res.getBadness() +
                            " currBadness: " +
                            nodes[currIndex].getBadness()
                    );
                if (res) console.log("resB: " + res.value);
            ...
            let currNode = new Node(
                nodes[currIndex],
                children[i],
                nodes[currIndex].getWeight() +
                    findWeight(nodes[currIndex].value, children[i])
            );
            console.log(
                "new node: " +
                    nodes[currIndex].value +
                    " " +
                    children[i] +
                    " weight: " +
                    nodes[currIndex].getWeight()
